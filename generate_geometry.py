...
class Circle(Shape):
    ...
    def draw(self, draw: ImageDraw.Draw):
        ...
        if self.alpha < 255:
            ...
            temp_draw.ellipse(bbox, fill=self.get_fill_color(), 
                            outline=self.get_stroke_color(), width=self.stroke_width)
            draw.im.paste(temp_img, (0, 0), temp_img)
        else:
...
class Rectangle(Shape):
    ...
    def draw(self, draw: ImageDraw.Draw):
        if self.rotation == 0:
            ...
            if self.alpha < 255:
                temp_img = Image.new('RGBA', draw.im.size, (0, 0, 0, 0))
                temp_draw = ImageDraw.Draw(temp_img)
                temp_draw.rectangle(bbox, fill=self.get_fill_color(), 
                                  outline=self.get_stroke_color(), width=self.stroke_width)
                draw.im.paste(temp_img, (0, 0), temp_img)
            else:
    ...
    def _draw_rotated_rectangle(self, draw):
        ...
        if self.alpha < 255:
            temp_img = Image.new('RGBA', draw.im.size, (0, 0, 0, 0))
            temp_draw = ImageDraw.Draw(temp_img)
            temp_draw.polygon(rotated_corners, fill=self.get_fill_color(), 
                            outline=self.get_stroke_color(), width=self.stroke_width)
            draw.im.paste(temp_img, (0, 0), temp_img)
        else:
            draw.polygon(rotated_corners, fill=self.color, 
                        outline=self.stroke_color, width=self.stroke_width)
...
class Triangle(Shape):
    ...
    def draw(self, draw: ImageDraw.Draw):
        if self.alpha < 255:
            temp_img = Image.new('RGBA', draw.im.size, (0, 0, 0, 0))
            temp_draw = ImageDraw.Draw(temp_img)
            temp_draw.polygon(self.points, fill=self.get_fill_color(), 
                            outline=self.get_stroke_color(), width=self.stroke_width)
            draw.im.paste(temp_img, (0, 0), temp_img)
        else:
...
class RegularPolygon(Shape):
    ...
    def draw(self, draw: ImageDraw.Draw):
        ...
        if self.alpha < 255:
            temp_img = Image.new('RGBA', draw.im.size, (0, 0, 0, 0))
            temp_draw = ImageDraw.Draw(temp_img)
            temp_draw.polygon(points, fill=self.get_fill_color(), 
                            outline=self.get_stroke_color(), width=self.stroke_width)
            draw.im.paste(temp_img, (0, 0), temp_img)
        else:
...
class Line(Shape):
    ...
    def draw(self, draw: ImageDraw.Draw):
        if self.alpha < 255:
            temp_img = Image.new('RGBA', draw.im.size, (0, 0, 0, 0))
            temp_draw = ImageDraw.Draw(temp_img)
            temp_draw.line([(self.x, self.y), (self.x2, self.y2)], 
